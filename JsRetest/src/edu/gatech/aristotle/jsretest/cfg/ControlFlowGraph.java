...
public class ControlFlowGraph<T>{
	...
	public boolean removeNode(T node){
		...
		for(ControlFlowEdge<T> e:new ArrayList<ControlFlowEdge<T> >(_getOutgoingEdges(node))){
			removeEdge(e.getSource(),e.getLabel());
		}
		for(ControlFlowEdge<T> e:new ArrayList<ControlFlowEdge<T> >(_getIncomingEdges(node))){
			removeEdge(e.getSource(),e.getLabel());
		}
		nodes.remove(node);
		return true;
